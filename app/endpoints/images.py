...
def upscale_image_opencv(pil_img: Image.Image, target_size: int = 1772) -> Image.Image:
...
def generate_images():
    ...
    try:
        ...
        for i, page in enumerate(book.pages):
            english_prompt = (
                f"A print-ready children's book illustration in a clean digital watercolor-inspired style. "
                f"Featuring {book.first_name} (a young girl with light brown hair in a pink dress with white lace) "
                f"{page.text}. The background is simple and magical, without texture or visual noise. "
                f"All lines are clean, colors are soft, and the composition is calm and balanced. "
                f"No paper-like effects or rough brush strokes. Single-page spread (1772x1772px). "
                f"Leave room at the bottom center for child-friendly sans-serif text."
            )
        ...
        return {"error": f"Fejl ved billedgenerering: {e}"}
